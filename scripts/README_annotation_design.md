...
- [x] 更新 [`README_prompt_engineering_spec.md`](README_prompt_engineering_spec.md) 的 system prompt 反映 marker token 设计 (§13, 2026-05-25)
...
## 12. 大规模标注 pilot 实验 insights (2026-05-25)

> 来源：v3-v3.3 × 5 iterations，Qwen2.5-VL-72B + MiMo-v2.5，同一组 5 ep DROID_100 子集。
> 详细数据见 [`REPORT_v33_final_comparison.md`](REPORT_v33_final_comparison.md)。

### 12.1 action 的三档精度（固化到 R6）

| 档位 | 适用场景 | 精度 | 例子 |
|------|---------|------|------|
| TIER A (grip) | grasp / release / retry | 只写 grip 动词 | `Close the gripper to grasp the marker` |
| TIER B (fine-tune) | 交互前最后 1-3 帧 / 姿态对齐 | axis name + 数字（gap-to-target） | `Pitch downward 39° to orient jaws perpendicular to the counter` |
| TIER C (transport) | 搬运 / 回收 / 自由移动 | 方向 + landmark 关系（定性） | `Lift the candy bar up to the shelf level` |

**关键发现**：模型很难自动区分 B 和 C —— v3.1 全给数字（72% axis-aware），v3.2/v3.3 几乎不给数字（6-9%）。**建议下一版在 keyframe metadata 里显式标 `near_interaction: true/false`** 帮模型区分。

### 12.2 stage 的空间唯一性原则

> "看 stage 文本就能在 10 几个 keyframe 中唯一定位当前帧"

实现方式：用 **空间 landmark** 而非通用描述。

| ✓ 好 | ✗ 差 |
|------|------|
| "The gripper is directly above the pot, holding the marker" | "The robot continues adjusting" |
| "Having risen above the sink rim, the arm is now level with the bottom shelf" | "The arm swings back and gains significant height" |
| "The gripper hovers 2 cm above the candy bar, ready to close" | "Fine-tuning position for grasp" |

指标：**stage-landmark-rate** 是比 axis-aware-action-rate 更强的质量信号。
...
| ✓ 允许（gap-to-target） | ✗ 禁止（raw delta） |
|---|---|
| "需要再 pitch 39° 才能垂直于桌面" | "已经移动了 22cm" |
| "夹爪悬停在目标上方 2cm" | "Δxyz=(+5cm,-3cm,+2cm)" |
| "还差 5° 就跟方块边缘对齐" | "旋转了 12°" |

含义不同：gap-to-target 描述"离目标配置还有多远"（对 policy 有用），raw delta 描述"已经运动了多少"（采样噪声，无指导价值）。

### 12.4 MiMo vs Qwen 对比结论

| 维度 | MiMo v3.3 | Qwen v3.3 |
|------|-----------|-----------|
| stage 图像理解 | ⭐ 更丰富、更多 landmark (70%) | 尚可 (30%) |
| stage 记忆引用 | "Having secured..." 自然 | 偶有 |
| action TIER 遵从度 | 中（偶尔不写 grip verb） | 高 |
| JSON 合规 | 中（曾有 parse 失败）| 高 |
| gap-to-target 数字 | ✓ 自然产出 | 很少 |
| think 触发 | 4% 偶有 | 0% |

**结论**：MiMo 的 **stage 质量更高**，Qwen 的 **结构合规性更好**。两者各有长处，不是简单 "换一个就好" 的关系。

### 12.5 gripper convention inversion 教训

2026-05-24 发现 DROID `observation.gripper_position` 是归一化命令值 [0=open, 1=closed]，但 detector 按 Franka 物理宽度 [0≈closed, 0.08≈open] 假设 → **所有 keyframe type 标反了**。

这导致 Qwen 的第一批输出（v1）看起来"完全不能用"——实际上模型一直在忠实执行我们给的错误标签。修了 convention 后 v2 mode-A 立刻 10/10 pass。

**教训**：数据 convention 错误比模型能力不足更容易被误诊为"模型太差"。任何新数据源必须先验证 gripper/action 的语义方向。

### 12.6 下一步（建议）

1. **显式 TIER B tagging**：在 keyframes_meta 里加 `near_interaction: true` 给 grasp/release 前后 1-2 帧 → 帮模型稳定切档
2. **MiMo stage + Qwen audit pipeline**：用 MiMo 做初始标注（stage 质量高），Qwen 过一遍 audit + 自动修正结构问题
3. **50-100 ep 放量验证**：当前 5 ep 太少看不到 retry case → think 字段无法验证
4. **训练侧接入**：v3.3 schema 已稳定，可以写 LAP cascade dataset class 消费 JSONL
